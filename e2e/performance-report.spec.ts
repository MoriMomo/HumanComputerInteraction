...
function generateHtmlReport(data: any): string {
    const { customMetrics, timestamp, url, viewport } = data;
...
<body>
    <div class="container">
        ...
        <div class="summary">
            <h3>Summary</h3>
            <p>
                Performance metrics collected from /showcase page. FPS measured during interaction (rotate + zoom simulation).
                Core Web Vitals captured during load and interaction phase.
            </p>
        </div>
        ...
        <div class="grid">
            ...
            <div class="card">
                ...
                <div class="metric">
                    ...
                    <span class="metric-value ${fps.average > 50 ? 'good' : fps.average > 30 ? 'warn' : fps.average > 0 ? 'bad' : 'warn'}">
                        ${fps.average.toFixed(1)} FPS
                    </span>
                </div>
                <div class="metric">
                    <span class="metric-label">Min FPS</span>
                    <span class="metric-value">${fps.min.toFixed(1)} FPS</span>
                </div>
                <div class="metric">
                    <span class="metric-label">Max FPS</span>
                    <span class="metric-value">${fps.max.toFixed(1)} FPS</span>
                </div>
                <div class="metric">
                ...
                <div class="bar">
                    ...
                    <span style="width: 50px; text-align: right; font-size: 0.9rem;">${(fps.average / 60 * 100).toFixed(0)}% of 60 FPS</span>
                </div>
                <p style="margin-top: 0.5rem; font-size: 0.85rem; color: #aaa;">
                    ${fps.average > 50 ? '✅ Excellent performance' : fps.average > 30 ? '⚠️ Good, but could improve' : '❌ Below target'}
                </p>
            </div>
            ...
            <div class="card">
                ...
                <p style="margin-top: 0.5rem; font-size: 0.85rem; color: #aaa;">
                    ${cwv.cls === 0 ? '✅ No layout shifts' : '⚠️ Minor layout shifts detected'}
                </p>
            </div>
            ...
            <div class="card">
                ...
                <div class="metric">
                    ...
                    <span class="metric-value">${(customMetrics.memory.jsHeapSizeLimit / 1073741824).toFixed(2)} GB</span>
                </div>
                <p style="margin-top: 0.5rem; font-size: 0.85rem; color: #aaa;">
                    ${customMetrics.memory.usedJSHeapSize < 50000000 ? '✅ Healthy' : '⚠️ Monitor usage'}
                </p>
            </div>
            ` : ''}
        </div>
        ...
        <div class="card" style="margin-top: 2rem;">
            ...
            <div style="color: #ccc; line-height: 1.8; font-size: 0.95rem;">
                ${fps.samples > 0
            ? `<p>✅ <strong>FPS Collection Success:</strong> ${fps.samples} samples captured (${(fps.samples * 1).toFixed(0)}s of measurement)</p>`
            : `<p>⚠️ <strong>FPS Collection:</strong> No samples captured</p>`
        }
                
                ${fps.average > 50
            ? `<p>✅ <strong>FPS Target Met:</strong> Average ${fps.average.toFixed(0)} FPS exceeds 50 FPS. Good performance on desktop.</p>`
            : fps.average > 30
                ? `<p>⚠️ <strong>FPS Acceptable:</strong> Consider enabling Draco compression (GLB) and KTX2 textures for additional headroom.</p>`
                : `<p>❌ <strong>FPS Below Target:</strong> Profile with DevTools and implement LOD variants for lower-end devices.</p>`
        }

                <p><strong>Next Steps:</strong></p>
                <ul style="margin: 1rem 0 0 1.5rem; color: #aaa;">
                    <li>Compress GLB with Draco (~60% reduction)</li>
                    <li>Use KTX2 textures (~50% additional reduction)</li>
                    <li>Add mobile LOD variants (simplified geometry)</li>
                    <li>Monitor real-user performance in production</li>
                </ul>
